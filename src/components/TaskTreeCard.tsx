		...
		>
			...
			{open && (
				<div className="space-y-2 border-t border-zinc-200 px-3 py-2.5 text-xs dark:border-zinc-800">
					{task.method && <Field label="方法">{task.method}</Field>}
					{task.condition && <Field label="條件">{task.condition}</Field>}
					{task.note && <Field label="注意">{task.note}</Field>}
					{task.result && <Field label="結果">{task.result}</Field>}
...
export default function TaskTreeCard({
	markdown,
	waitingConfirm,
	onConfirmStep,
}: TaskTreeCardProps) {
