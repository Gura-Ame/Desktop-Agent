...
export default function ChatMessage({
    ...
    return (
      <div className="mx-auto flex max-w-3xl min-w-0 gap-3 justify-start">
        ...
        <div className="min-w-0 max-w-[min(85%,42rem)] flex-1 space-y-1">
          {treeTime && (
            <span className="px-1 text-[10px] tabular-nums text-zinc-400">{treeTime}</span>
          )}
          <TaskTreeCard
  ...
  const blocks = parseMessageContent(msg.content, { isStreaming: !!msg.isStreaming });
          ...
          {editing ? (
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                <Button size="sm" variant="primary" onClick={() => saveEdit(false)}>
                  儲存
                </Button>
                <Button size="sm" variant="default" onClick={() => saveEdit(true)}>
                  儲存並重送
                </Button>
                <Button
                  size="sm"
