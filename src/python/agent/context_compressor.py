...
class ContextCompressor:
    ...
    def compress(self, call_llm_fn: Callable[[str, str], str],
        ...
        # 現有節點的 id 集合，用來做 MENTIONS 關聯的比對基準
        known_ids = set(self.memory_store.nodes.keys())
        ...
        for i, fact in enumerate(facts, start=1):
            ...
            # 若 fact 的 summary/detail 裡提到了已知節點 id，建立 MENTIONS 關聯——
            # 這讓 Retriever 日後透過圖遍歷找到這些被壓縮過的結論，
            # 而不是讓它們孤立在 MemoryStore 裡查不到。
            fact_text = (fact.get("summary", "") + " " + fact.get("detail", "")).lower()
            for known_id in known_ids:
                if known_id.lower() in fact_text:
                    try:
                        self.memory_store.add_relation(fact_id, "MENTIONS", known_id)
                    except KeyError:
                        pass  # 目標節點不存在時跳過，不中斷壓縮流程
