...
from tools.code_graph import CodeGraphBuilder  # noqa: E402
...
def test_unresolved_external_calls_are_skipped_without_error(builder: CodeGraphBuilder, store: MemoryStore, root: str):
    """呼叫目標在外部函式庫時，自動建立 ExternalRef 節點，不報錯、不產生查無此節點的斷鏈。"""
    result = builder.build_project(root)  # 不該丟例外
    assert store.get_node("main.run") is not None
    callees = builder.find_callees("main.run")
    assert "os.path.join" in callees or "unknown_external_lib.do_something" in callees
    # 確認每個被呼叫的節點都真的存在於 store 中（不是斷鏈）
    for c in callees:
        node = store.get_node(c)
        assert node is not None
        assert node.type == "ExternalRef"
