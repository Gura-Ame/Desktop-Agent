...
function test_header_line_does_not_eat_first_task() {
  ...
  assert.strictEqual(tasks.length, 1, `應該解析出 1 個任務，實際: ${tasks.length}`);
...
function test_depth_reflects_real_indentation_not_dot_count_in_id() {
  ...
  assert.strictEqual(byId['TASK-1.impact1'].depth, 0, 'impact-check 任務結構上是獨立任務，不該被縮排');
  assert.strictEqual(byId['TASK-2'].depth, 0);
  assert.strictEqual(byId['TASK-2.1'].depth, 1, '真正的拆解子任務應該要縮排一層');
  console.log('[PASS] test_depth_reflects_real_indentation_not_dot_count_in_id');
}
